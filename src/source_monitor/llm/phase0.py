...
from source_monitor.llm.cache import ModelMeta, load_model
from source_monitor.llm.config import HARD_TASK, PRIMARY_TASK, Phase0Config, TaskConfig
from source_monitor.llm.corruption import CorruptionRecord, inject_all_types
...
def run_model_experiment(
    ...
    corrupt_scores = {
        agg: {t: [] for t in corruption_types} for agg in ("mean", "max", "slot_only")
    }
    ...
    for task in tasks:
        ...
        if scoring == "contrastive":
            clean_span_scores = contrastive_slot_scores(
                model, tokenizer, clean_trace, device
            )
        else:
            clean_ids, clean_spans = tokenize_with_provenance(
                tokenizer, clean_trace, device
            )
        ...
        for corr_type in corruption_types:
            ...
            if scoring == "contrastive":
                corr_span_scores = contrastive_slot_scores(
                    model, tokenizer, record.trace, device
                )
            else:
                corr_ids, corr_spans = tokenize_with_provenance(
                    tokenizer, record.trace, device
                )
                ...
                if score.is_corrupted:
                    ...
                    for agg in ("mean", "max", "slot_only"):
                        ...
                        if corr_step in clean_step_map:
                            twin_val = getattr(
                                clean_step_map[corr_step], f"{agg}_neglogp"
                            )
                            paired_deltas[agg][corr_type].append(val - twin_val)
                elif step < corr_step:
                    # Before corruption
                    for agg in ("mean", "max", "slot_only"):
                        before_scores[agg][corr_type].append(
                            getattr(score, f"{agg}_neglogp")
                        )
                elif step > corr_step:
                    # Cascade (after corruption)
                    for agg in ("mean", "max", "slot_only"):
                        cascade_scores[agg][corr_type].append(
                            getattr(score, f"{agg}_neglogp")
                        )
    ...
    for agg in ("mean", "max", "slot_only"):
        ...
        for corr_type in corruption_types:
            ...
            matched_genuine = [
                val
                for val, surf in zip(
                    clean_genuine_scores[agg], clean_genuine_surfaces, strict=False
                )
    ...
    for model_name in model_names:
        ...
        for task_cfg in config.task_configs:
            for seed in config.seeds:
                print(
                    f"Running {task_cfg.label} task | seed {seed} | traces {n_traces}..."
                )
                ...
                for agg in ("mean", "max", "slot_only"):
                    ...
                    print(
                        f"    [{agg}] genuine_clean_avg = {res.mean_genuine_clean:.3f}"
                    )
