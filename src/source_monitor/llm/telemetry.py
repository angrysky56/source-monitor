...
def contrastive_slot_scores(
    ...
    from source_monitor.llm.task_render import CONTAINER_NAMES, Trace, Turn
    ...
    for turn_idx, turn in enumerate(trace.turns):
        ...
        for c in candidates:
            ...
            cand_ids, cand_spans = tokenize_with_provenance(
                tokenizer, cand_trace, "cpu", skip_prefix_check=True
            )
        ...
        for i, c in enumerate(candidates):
            ...
            if content_tokens:
                ...
                content_logits = outputs.logits[
                    i, [pos - 1 for pos in content_tokens], :
                ].float()
            ...
            mean_lp = (
                sum(neg_logps_content) / len(neg_logps_content)
                if neg_logps_content
                else 0.0
            )
            ...
            if slot_tokens:
                ...
                slot_logits = outputs.logits[
                    i, [pos - 1 for pos in slot_tokens], :
                ].float()
        ...
        ), f"Emitted location {emitted_loc!r} not in candidates"
